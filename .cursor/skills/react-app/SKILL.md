...
4. コンポーネントは `.tsx`。ルール `javascript` を守る（`const`/`let`、レキシカル環境、DOM をクラスで掴む場合は `js-` 接頭辞、`innerHTML` 禁止）。イベントは合成イベント。CSS のインデント（タブサイズ）は **4 スペース**。ネガティブマージン（`margin` の負値）は使わない。
